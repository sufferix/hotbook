...
public class RoomController {
    ...
    public ResponseEntity<Response> addNewRoom(
        ...
        if (photo == null || photo.isEmpty() || roomType == null || roomType.isBlank() || roomPrice == null || roomType.isBlank()) {
            ...
            response.setMessage("Необходимо указать значения для всех полей(Фото, Тип комнаты, Цена за ночь)");
    ...
    public ResponseEntity<Response> getAvailableRoomsByDateAndType(
        ...
        if (checkInDate == null || roomType == null || roomType.isBlank() || checkOutDate == null) {
            Response response = new Response();
            response.setStatusCode(400);
            response.setMessage("Необходимо указать значения для всех полей(Дата Заезда, Тип комнаты,Дата выезда)");
            return ResponseEntity.status(response.getStatusCode()).body(response);
        }
